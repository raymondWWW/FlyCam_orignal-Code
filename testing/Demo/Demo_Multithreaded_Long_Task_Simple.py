    ...
    while True:
        ...
        if event in (sg.WIN_CLOSED, 'Exit'):
            ...
            if len(values['-SECONDS-']) != 0:
                seconds_value = float(values['-SECONDS-'])
            else:
                seconds_value = 1
            print('Thread Starting! Long work....sending value of {} seconds'.format(seconds_value))
            timeout = 100 if values['-ONE CHUNK-'] else None
            thread = threading.Thread(target=long_operation_thread, args=(seconds_value, window), daemon=True)
            thread.start()
            if values['-ONE CHUNK-']:
