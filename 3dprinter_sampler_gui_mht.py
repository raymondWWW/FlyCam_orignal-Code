...
import module_color_sensor as CS
...
def main():
    ...
    # Color Sensor Tab
    tab_6_layout = [ [sg.Button("Setup Color Sensor"),sg.Button("Get Color")],
    ...
    layout = [ [sg.Image(filename='', key='-IMAGE-')],
               [sg.TabGroup([[sg.Tab("Tab 1 (Exp)", tab_1_layout, key="-TAB_1_KEY"),
                              ...
                              sg.Tab("Tab 6 (Color Sensor)", tab_6_layout)]])
               ],
    ...
    while True:
        ...
        # Color Sensor stuff
        if event == "Setup Color Sensor":
            print("You pressed Setup Color Sensor")
            CS.color_sensor_setup()
            CS.set_100_output()
            CS.setup_temp_folder_and_csv()
        elif event == "Get Color":
            print("You pressed Get Color")
            colors = CS.get_rbgc_prf(CS.TIME_TO_WAIT)
            print(colors)
        elif event == "Save Color":
            loc_dict = get_current_location2()
            CS.save_color_to_csv(loc_dict)
            print("You pressed Save Color")
        elif event == "Stop Color Sensor":
